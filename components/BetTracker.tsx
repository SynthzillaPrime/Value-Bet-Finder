...
export const BetTracker: React.FC<Props> = ({
  ...
  const checkClosingLine = async (bet: TrackedBet) => {
    if (new Date() < new Date(bet.kickoff)) {
      alert(
        "Match hasn't started yet. Closing line is only available after kickoff.",
      );
      return;
    }
    ...
    if (result) {
      ...
      alert(
        "Could not find closing line in history. It might be too early, or data is unavailable on your plan.",
      );
    }
  };
