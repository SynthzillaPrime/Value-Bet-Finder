import { LEAGUES, BOOKMAKERS, EXCHANGES } from '../constants';
import { MatchResponse, BetEdge, HistoryResponse, ExchangeOffer } from '../types';

/**
 * Removes the vigorish (margin) from a set of odds to find the "fair" probability/price.
 * Uses the multiplicative method.
 */
export const stripVig = (outcomes: { price: number; name: string }[]): Record<string, number> | null => {
 ...
 */
const fetchLeagueOdds = async (apiKey: string, leagueKey: string): Promise<FetchResult> => {
 ...
 */
export const fetchOddsData = async (apiKey: string, selectedLeagues: string[]): Promise<{ matches: MatchResponse[], remainingRequests: number | null }> => {
  const fetchPromises = selectedLeagues.map(leagueKey => fetchLeagueOdds(apiKey, leagueKey));
  const results = await Promise.all(fetchPromises);
  
  const flatMatches = results.flatMap(r => r.data);
  const remainingCounts = results.map(r => r.remaining).filter((r): r is number => r !== null);
  const remainingRequests = remainingCounts.length > 0 ? Math.min(...remainingCounts) : null;
 ...
 */
export const calculateEdges = (matches: MatchResponse[], minEdge: number = 0): BetEdge[] => {
  const allBets: BetEdge[] = [];
  
  // Filter out matches that have already started
  const now = new Date();
  const upcomingMatches = matches.filter(m => new Date(m.commence_time) > now);
    ...
    for (const pinnMarket of pinnacle.markets) {
      ...
      for (const outcome of pinnMarket.outcomes) {
          ...
          for (const exConfig of EXCHANGES) {
             const exchangeBookie = match.bookmakers.find(b => b.key === exConfig.key);
             if (!exchangeBookie) continue;

             const exMarket = exchangeBookie.markets.find(m => m.key === pinnMarket.key);
             ...
             const effectiveOdds = 1 + ((exOutcome.price - 1) * (1 - exConfig.commission));
 ...
 */
export const scanForEdges = async (apiKey: string, selectedLeagues: string[], minEdge: number = 0): Promise<{ bets: BetEdge[], remainingRequests: number | null }> => {
    const { matches, remainingRequests } = await fetchOddsData(apiKey, selectedLeagues);
 ...
 * (Pinnacle No-Vig)
 */
export const fetchClosingLineForBet = async (apiKey: string, bet: BetEdge): Promise<ClosingLineResult | null> => {
  ...
  try {
    ...
    const match = json.data.find(m => m.id.split(':')[0] === bet.id.split('-')[0] || m.home_team === bet.homeTeam);
